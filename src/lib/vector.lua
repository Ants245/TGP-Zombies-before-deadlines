...
module.Vector2D = function (ix, iy)
  ...
  function v2d:normalize(vec1, vec2)
    local temp = module.Vector2D(0,0)
    local mag = ((vec1:getX() * vec1:getX()) + (vec1:getY() * vec1:getY()))
    self.v.x = (vec1:getX() / mag)
    self.v.y = (vec1:getY() / mag)
  end
